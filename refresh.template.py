...
def _parse_headers_from_makefile_deps(d_file_content: str, source_path_for_sanity_check: typing.Optional[str] = None):
    ...
    assert target.endswith(('.o', '.obj', '.pcm')), "Something went wrong in makefile parsing to get headers. The target should be an object file. Output:\n" + d_file_content
...
_get_files.cpp_source_extensions = ('.cc', '.cpp', '.cxx', '.c++', '.C', '.CC', '.cp', '.CPP', '.C++', '.CXX', '.ii', '.cppm', '.cxxm', '.c++m', '.pcm')
